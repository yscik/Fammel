...
class lime {
	...
	function build_parser() {
		...
		$a = $this->rule_table();
		$qi = $initial->id;
		return $this->lang->ptab_to_class($this->parser_class, $this->parser_extends, compact('a', 'qi', 'i'));
	}
	function rule_table() {
		$s = array();
...
class lime_language_php extends lime_language {
	...
	function ptab_to_class($parser_class, $parser_extends, $ptab) {
		$code = "class $parser_class extends $parser_extends {\n";
...
function lime_bootstrap() {
   ...
   $lime->parser_extends = 'lime_parser';
	$rhs = array();
